...
const Chat = () => {
  ...
  // Settings
  const [disappearingMode, setDisappearingMode] = useState("instant"); // instant (default), off, 24h
  ...
  useEffect(() => {
    ...
    // Fetch chat metadata (wallpaper + settings)
    ...
    const unsubscribeMsgs = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        // "24 Hour Delete" Logic check on read
        // Note: Realistically, this should be a backend trigger, but simulating client-side lazily
        if (disappearingMode === "24h" && data.createdAt) {
             const now = new Date();
             const msgTime = data.createdAt.toDate();
             const diffHours = (now - msgTime) / (1000 * 60 * 60);
             if (diffHours >= 24) {
                 // Skip adding to list, let's behave as if it's gone.
                 // Ideally we delete it here too to clean up.
                 // deleteDoc(doc.ref); 
                 return; 
             }
        }
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (file) {
        // Optimistic UI for Image (Speed perception)
        // If image, we could show local blob immediately.
        // For simplicity, we just rely on standard flow but use uploadBytesResumable
             ...
             <div className="relative">
                 <button onClick={() => setShowSettings(!showSettings)} className="hover:text-emerald-200" title="Settings">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                        <path fillRule="evenodd" d="M11.078 2.25c-.917 0-1.699.663-1.85 1.567l-.091.549a.798.798 0 0 1-.517.608 7.45 7.45 0 0 0-.478.198.798.798 0 0 1-.796-.064l-.453-.324a1.875 1.875 0 0 0-2.416.2l-.043.044a1.875 1.875 0 0 0-.205 2.42l.33.452c.196.27.202.63.023.91a7.424 7.424 0 0 0-.27.564.798.798 0 0 1-.699.508l-.547.052A1.875 1.875 0 0 0 2.25 11.08v1.84c0 .917.663 1.699 1.567 1.85l.549.091c.28.047.518.25.608.517.06.162.127.321.198.478a.798.798 0 0 1-.064.796l-.324.453a1.875 1.875 0 0 0 .2 2.416l.044.043a1.875 1.875 0 0 0 2.42.205l.452-.33c.27-.196.63-.202.91-.023.184.108.373.2.564.27a.798.798 0 0 1 .508.699l.052.547A1.875 1.875 0 0 0 11.08 21.75h1.84c.917 0 1.699-.663 1.85-1.567l.091-.549a.798.798 0 0 1 .517-.608c.162-.06.321-.127.478-.198a.798.798 0 0 1 .796.064l.453.324a1.875 1.875 0 0 0 2.416-.2l.043-.044a1.875 1.875 0 0 0 .205-2.42l-.33-.452a.798.798 0 0 1-.023-.91c.108-.184.2-.373.27-.564a.798.798 0 0 1 .699-.508l.547-.052A1.875 1.875 0 0 0 21.75 12.92v-1.84c0-.917-.663-1.699-1.567-1.85l-.549-.091a.798.798 0 0 1-.517-.608 7.45 7.45 0 0 0-.478-.198.798.798 0 0 1-.796.064l-.453.324a1.875 1.875 0 0 0-2.416-.2l-.043.044a1.875 1.875 0 0 0-.205 2.42l.33.452c-.196.27-.202.63-.023.91a7.424 7.424 0 0 0 .27-.564.798.798 0 0 1 .699-.508l.547-.052A1.875 1.875 0 0 0 12.92 2.25h-1.84Z" clipRule="evenodd" />
                    </svg>
                 </button>
                 {showSettings && (
                     <div className="absolute right-0 top-8 bg-white text-gray-800 rounded shadow-lg p-3 w-48 z-50 text-xs">
                         <p className="font-bold mb-2 border-b pb-1">Disappearing Msg:</p>
                         ...
                         <button onClick={() => updateDisappearingMode('24h')} className={`block w-full text-left py-1 ${disappearingMode === '24h' ? 'text-emerald-600 font-bold' : ''}`}>24 Hours</button>
